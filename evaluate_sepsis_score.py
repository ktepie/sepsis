...
def evaluate_sepsis_score(label_directory, prediction_directory):
    ...
    for k in range(num_files):
        ...
        if not (len(labels) == len(predictions) and len(predictions) == len(probabilities)):
            raise Exception('Numbers of labels and predictions for a file must be the same. Your labes has {} files and predictions has {}.'.format(len(labels), len(predictions)))
